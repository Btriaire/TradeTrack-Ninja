    ...
    <div className="bg-dark-800 rounded-xl p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-white">Simulateur Ordre LCL Bourse</span>
        </div>
        <button
          onClick={() => setShowTarifs(!showTarifs)}
          className="text-xs text-slate-500 hover:text-white transition-colors flex items-center gap-1"
        >
          <Info size={12} /> Grille tarifaire
        </button>
      </div>
        ...
        <div className="mb-4 p-3 bg-dark-700 rounded-lg text-xs space-y-1.5">
          ...
          {tarifs.courtage_internet?.map((t: any, i: number) => (
            <div key={i} className="flex justify-between text-slate-400">
              <span>{t.tranche}</span><span className="text-yellow-400">{t.tarif}</span>
            </div>
          ))}
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div>
          ...
          <input
            ...
            className="w-full bg-dark-700 text-white text-sm rounded-lg px-3 py-2 outline-none border border-dark-600 focus:border-accent-blue/50"
          />
        </div>
        <div>
      ...
      {/* Montant brut */}
      <div className="bg-dark-700 rounded-lg px-3 py-2 mb-4 flex justify-between items-center">
      ...
      <div className="flex flex-wrap gap-3 mb-4 text-xs">
        ...
        <label className="flex items-center gap-1.5 text-slate-400 cursor-pointer">
          <input type="checkbox" checked={ttf} onChange={e => setTtf(e.target.checked)}
            className="accent-blue-500" disabled={!actionFr} />
      ...
      <button
        ...
        className="w-full bg-accent-blue hover:bg-blue-600 disabled:opacity-50 text-white font-semibold rounded-lg py-2.5 text-sm transition-colors"
      >
      ...
      {fees && (
        <div className="mt-4 space-y-2">
          ...
          <div className={`rounded-lg p-3 text-sm font-bold flex justify-between items-center ${
            direction === 'achat' ? 'bg-green-500/10 border border-green-500/20' : 'bg-red-500/10 border border-red-500/20'
          }`}>
            <span className="text-slate-300">
              {direction === 'achat' ? 'Montant total à débiter' : 'Montant net à créditer'}
            </span>
            <span className={direction === 'achat' ? 'text-green-400' : 'text-red-400'}>
              {direction === 'achat'
